...
class GiftcardStatsExpired extends Component {
    ...
    async getGiftcardsExpired() {
        try {
            const response = await fetch(GIFTCARDSEXPIRED_URL)
    ...
    async getTotals() {
        try {
            const response = await fetch(GIFTCARDSEXPIREDTOTALS_URL)
        ...
        return(
            <ScrollView contentContainerStyle={styles.container}>
                <View style={styles.area}>
                    <Text style={[styles.text, {fontSize: 18}]}>Statistik over udløbne gavekort</Text>
                </View>
...
export default GiftcardStatsExpired;
